...
void DistanceConstraint::ApplyImpulse()
{
	...
	float test = pnodeB->GetAngularVelocity().y;
	...
	if (constraintMass > 0.0f)
	{
		...
		Vector3 ans1 = Vector3::Cross(r1, abn * jn);
		Vector3 ans2 = Vector3::Cross(r2, abn * jn);
		//ans1.Normalise();
		//ans2.Normalise();
		//ans1 = ans1 * 0.0000001;
		//ans2 = ans2 * 0.0000001;

		pnodeA->SetAngularVelocity(pnodeA->GetAngularVelocity() + pnodeA->GetInverseInertia() * ans1);
		
		pnodeB->SetAngularVelocity(pnodeB->GetAngularVelocity() - pnodeB->GetInverseInertia() * ans2);
	}
}